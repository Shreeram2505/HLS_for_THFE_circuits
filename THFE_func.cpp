...
void printBinary(std::default_random_engine& engine) {
    // Serialize the engine's state to a string
    std::ostringstream oss;
    oss << engine;

    std::string serializedState = oss.str();

    // Print the serialized state in binary format
    for (char c : serializedState) {
...
// int main() {
//     // Create and seed a default random engine
//     std::default_random_engine engine(12345);

//     // Generate some random numbers to change the engine's state
//     for (int i = 0; i < 10; ++i) {
//         engine();
//     }

//     // Print the binary representation of the engine's state
//     std::cout << "Binary representation of the engine's state:" << std::endl;
//     printBinary(engine);
